...
if __name__ == "__main__":
    ...
    config_file = os.path.join(args.config_name, 'config_'+args.config_type+'.json')
    ...
    for class_name in col_dict.keys():
        col_ner = nlp_gen(col_dict[class_name]['ner2nlp_col'],col_dict[class_name]['ner_list'])
        col_dwb = dwb_gen(
        ...
        dwb_class.append(class_name)
    for no_ner_table in config['ner_model_dict'].keys():
        print(config['ner_model_dict'][no_ner_table])
        if config['ner_model_dict'][no_ner_table] is None:
            ner_table_name = nlp_dict.get(no_ner_table)
            print(ner_table_name)
            nlp.append(sql_template_nlp.substitute(class_name='nlp '+no_ner_table,
                                               table_name=ner_table_name, ner_col='', config_name=args.class_name))

    for no_dwb_class in dwb_dict.keys():
        print(config['dwb_table_dict'][no_dwb_class])
        if no_dwb_class not in dwb_class:
            dwb_table_name = dwb_dict.get(no_dwb_class)
            print(dwb_table_name)
            dwb.append(sql_template_dwb.substitute(class_name='dwb '+no_dwb_class,
                                               table_name=dwb_table_name,dwb_columns = columns_dwb, dwb_col='', config_name=args.class_name))

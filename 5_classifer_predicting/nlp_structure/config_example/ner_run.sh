...
mc cat minio/model/bankV1NerModel0530.tgz >> bankV1NerModel0530.tgz
echo "解压"
tar xvf rust_server.tgz && rm rust_server.tgz
tar xvf predict.tgz && rm predict.tgz
tar xvf bankV1NerModel0530.tgz && rm bankV1NerModel0530.tgz
# mv bank_classfiy predict/  对于分类任务，需要将模型文件夹移动到predict文件夹下
echo "启动"
gpu_list=`get_list GPU_NUM WORK_NUM`
# 启动模型Work
# ner任务:
cd $DIR/predict && bash start_predict_worker.sh -s 9001 -d 9003 -g "$gpu_list" -l ./ -m $DIR/bankV1NerModel0530/saved_model  # -m指定模型目录
# 分类任务

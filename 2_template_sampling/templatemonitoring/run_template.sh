...
ding_notice(){
      if [[ $# != 1 ]];then
          return 1
            ...
            echo "param is ~~~~~~~~~~~~"$1
      curl -X POST -H 'Content-Type: application/json' -d '{"accessToken": "36b4f1252f4482cbb592772f513e76bd3284c50bc5bfb534dbfc3a4d877d7da3","content": "'$1'" }' "http://10.10.15.52:8888/conch/notice/ding"
      return 0
    }
...
run_template --the_date $the_date --config_file $configfile
if [[ $? != 0 ]]; then
    echo "${start} write base data failed"
    exit 1
fi
notice="${the_date}新模板聚类完成"
ding_notice "$notice"  

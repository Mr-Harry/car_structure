...
class MinHash:
    def __init__(self, features, permutations, bands, seed=1201, hash_func=_hash_func):
        self.seed = seed
        ...
        self.signature = self._hashing(features)
        self.meta_hash = self._banding()
    ...
    def _banding(self):
        band_hashes = np.array([self.signature[start:end] for start, end in self.hash_ranges])
